...
Fyi - chatgpt helped me do this. i gave it some examples of my dataframe (before we went over this in class) and then told it how I wanted the data to look.
...
#chatgpt helped me here as well - I had to work through one problem at a time with it, i kept getting NA values or wrong values in the columns. I kept telling it "that worked, but now I have NA values in the total_race and latino columns" etc. and it worked through it with me, spit this out
...
mutate(cte18 = str_remove_all(cte18, "%"), cte22 = str_remove_all(cte22, "%")) |>
  mutate(cte18 = as.numeric(cte18), cte22 = as.numeric(cte22))

cte_changes <- cte_changes |>
  mutate(cte_change = cte22-cte18)
...
#cte_changes" - it told me I was doing my mutate wrong and I felt dumb for asking

```

```{r}
cte_changes |>
  arrange(desc(cte_change))
```
...
cte_changes |>
  arrange(cte_change)
```
Let's make a bar graph of this.
```{r}
write_csv(cte_changes, "cte_changes.csv")
knitr::include_graphics("cte_participation_growth.png")
```
...
Story Idea 2:
Lower-income counties tend to have higher CTE participation rates for high schoolers. I'd like to find out what the top programs are for these areas, what these students are interested in pursuing, how they chose their career path, etc. Students from higher-income counties are more likely to go to college (this is sort of a no brainer). However, there is often a lack of education for students, especially those from lower-income communities, on how they can make college affordable. I'd like to look at what barriers students from lower-income communities face in going to college and why they chose CTE over college. For our article on the college and career readiness pillar of the blueprint, I'd also like to look at if and how career counselors plan on addressing college affordability to help students make more informed choices about college.

Story Idea 3:
Somerset, Baltimore and Wicomico counties have seen the biggest jump in CTE participation from 2018 to 2022. PG, Talbot and Baltimore city are falling behind with a decrease in CTE participation. Given the state's push to get at least 45% of high schoolers participating in job training of some kind by 2025, what kind of effort is being made in the counties that are sending more students to CTE programs? What about the counties that are sending fewer kids than in 2018? More importantly, how do schools with low participation plan to bolster their student support for those who want to pursue career training over a four-year degree?

Note to Derek: 
I realized I would have been much better off looking at data I could play with more than aggregate data. Sorry my data anaylsis is lacking - I went into this with the mindset of wanted to see how different factors impact each other, which I think I did, but I now see why you wanted us to have more data rows. On the bright side, I definitely got some good information for my blueprint article!
