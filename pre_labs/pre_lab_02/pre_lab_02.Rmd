...
**Answer**
It's important to make a new variable so that we can easily pull this information up when we want to review it or do something further with it - e.g. if we were looking at just this data, we can now go into "selected journalism courses" in our global environment and look at the simplified data set.
...
**Answer**
The & and comma are exclusive and only retrieve rows that match both criteria together, while OR or | is not exclusive and returns data that matches either theatre OR 15 seats. It gives us 41,011 results vs 231.
...
**Answer:**
The column now says "date" instead of "chr" and it's formatted differently (year-month-date)
...
**Answer**
It looks like R has grouped all the expenditures per month per year, labeled with the first day of each month, and then summarized the total amount of the expenditures logged for each month.
...
**Answer**
This code created a separate data frame called general_22, then created a new column (mutate) with the total number of votes that added up the numbers in each column listed, then added a second column for the percent of votes that went to moore that divided the number in the moore column by the number in total_votes column.
...
**Answer**
The data is now easier to read because we turned our decimals into percentages and sorted with the counties with the highest percentage rate for Moore at the top. It chnaged because we mutated the pct_moore column and multiplied the number by 100.
...
I think it depends on what info you're looking for. In some ways, yes - I was surprised the county where moore was least popular still went for him by 22% - he's obviously popular in MD as he won at least a third of the votes in nearly every county out of five candidates. However, on the flip side, some people may be more interested in knowing that the top four counties he was most popular in (howard, montgomery, baltimore city, and prince george's) he won by 69% to 89%. These are some of the largest counties in terms of number of voters as well.
...
**Answer**
Two functions similar to str_to_upper are:
1. str_to_lower which converts everything to lowercase
2. str_to_title which capitalizes the first letter of each word
...
**Answer**
I think the mutate code is creating a new column for state, then telling R to populate that column with MD when it sees Maryland in the address, and so on. It also set the default to NA so that if it doesn't find any of these state names we've specified, it says NA in the state column.
...
**Answer**
Yes, mostly. It's summing the expenditures for each state, and it makes sense that MD would be the top contributor to expenditures for MD. I'm kind of surprised that NA and WA both had high expenditures because why would Washington state have any investment? I looked at the data and it looks like it's pulling both Washington DC and Washington state into the WA state category.
...
1. Now that I can mutate the data, I can better answer my previous question from my pre-lab 1 - how did class availability change over time? More specifically, what classes are more widely available in 2023 vs. pre-covid (2019, e.g.)?

2.  What expense category received the most funding in the maryland expenses data?

3.  How much funding went to candidates themselves? Each candidate?
