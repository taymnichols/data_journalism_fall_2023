...
Line one creates a new dataframe, line 2 creates a new month column and pulls the month from the date column. I think the label=true gives it the name of the month label instead of the number of the month.
Then we grouped the entries by month and counted the number of calls or rows per month. Then we sorted them in order of the highest call rate.
...
Literally nothing :') Just a big blank space.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
We see a bar chart! We have the total calls on the y axis and the month on the x axis.
...
###notes to self - weight is the size of the bars?
```
...
We reordered our X value to be in order of total calls.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
###NOTES: coord_flip flips it on it's side
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
Honestly barely anything but it did change the background and it's a little cleaner.
...
The whole background is now kind of a bluish color and the month names/count labels are larger.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
We have a title, we fixed the x and y axis labels and we included our source!
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a list of the total calls by date.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
A very busy line chart showing that the majority of days have between 5-15 calls. Over 20 and under 2.5 are outliers.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
Now we can see the differences a little bit better because we split it out by week but we can't really read the dates.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
Now we can read the weeks better because the labels are slanted/tilted.We can see that there are some peaks at the beginning of some months.
