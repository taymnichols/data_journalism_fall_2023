...
**Answer**
Column names aren't clean and the date col is a character. However it looks very ready to be cleaned and pretty usable!
...
**Answer**
Not sure what I would want to add about these but would be good to have a column maybe referencing what it's about (e.g. public hearing, meeting, funding, etc)
...
tags there are in the HTML source? **Answer** a few - looks like there's a list for the menu, a list for the social media links, and a list for the press releases.

```{r}
osp_results |> html_elements('li')
#note to self - html_element grabs only the FIRST element you specify, html_elements calls all tags
```
...
we get a list of a bunch of different actions, plus dates and what look like notices of some kind

```{r}
osp_results |> html_elements('li') |> html_text()
#note to self - pulls just the ahref text
```
...
Now we have a dataframe! Still contains information we don't need though.

```{r}
releases <- osp_results |> html_elements('li') |> html_text() |> as_tibble()
#as tibble takes this text and turns it into a data frame
```
...
#uses html scaffolding - e.g. .listing-item or #listing-item
```
