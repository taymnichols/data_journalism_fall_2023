...
We got a list of dates, headlines/titles, urls, and the text of the article.
```{r}
...
it finds all the unique instances of a word occuring, but it is case sensitive.
...
it gave us a df of all the unique words, all lowercased. 
...
not really - it is a bar chart with the top words used, but they are mostly connector words like the or and.
...
yes! now we can see actually interesting words and not words we know are already going to be included like maryland.
...
2021 and 22 were pretty covid and health focused. 2022 deviated from covid funding talks of the year prior and focused on community programs and baltimore? 20223 was talking more about legislation/abortion access I think, and then firing someone in congress maybe? 
```{r}
unique_words_2021 |>
  anti_join(stop_words) |>
  group_by(word) |>
...
**Task** Run the following code and describe the results. **Answer**
Focused on covid 19 pandemic, human rights, the american rescue plan, healthcare, chesapeake bay and baltimore.
...
**Task** Run the following code and describe the results. **Answer**
lots more maryland specific things - PG, baltimore, chesapeake bay, etc. and maybe some specific project locations?
```{r}
releases |>
...
**Task** Run the following code and describe the results. **Answer**
More focus on human rights and equal rights, looks like he was probably talking about abortion access and maybe racial equity in some capacity? Also oral /dental health included in healthcare.
...
Wow this is really interesting. It says critical is a negative word which I would disagree with (depends on context obviously but I would imagine it's not negative here). Trump it also says is positive lol.
