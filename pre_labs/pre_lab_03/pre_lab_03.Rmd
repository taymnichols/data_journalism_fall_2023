...
It says there's a parsing issue and we can use the problems function to get more info. I ran problems and it gave me a bunch of rows with XXXXs where it thinks it should be a number column.
...
**Answer**
Nope! It fixed the parsing error.
...
**Answer**
Vendor name and Date are both character fields, and amount is a double/number field.
```{r}
glimpse(payments)
```
...
**Answer**
There are 549 zip codes with less than 5 characters, but thousands of instances of this happening throughout the data. I would assume it's probably error when entering data, missing data, or zip codes from a different country.
...
**Answer**
No loan data for 2009, one year there's data for L and for Loans, a handful of NAs, and one contract.

Note to self:
group by multiple categories, then summarize and count to get the # of things in those two categories *combined* 
...
**Answer**
A few entries that are under 300 dollars, mostly things over 300. A lot of grants are described as "CASH FOR COMMOD - CHILDCARE," funding for individual people, some zip codes are 9 digit.
...
Note to self: 
CLEAN NAMES ONLY WORKS ON COLUMN NAMES, does not fix data
...
**Answer** 
we're creating a new dataframe called cleaned_md_grants_loans which contains the original md_grants_loans. We ran clean_names to unify the column names, we renamed grantor to source, we then mutated the source, grantee, and description columns and told R to use str_to_upper to make all of the characters in these columns uppercase.
...
**Answer**
There are 25 sets of duplicates. I guess 17,764 duplicates are possible if every row is a duplicate row.
...
**Answer**
The new dataframe has 17,740 rows.
...
**Answer**
We added a new column with the 5 digit zip code, but kept the old column with the original zips as well.

###NOTE TO SELF:
mutuate means I want a new columun, zip5 is new col name, str_sub function says give me a portion of this column, start 1L means first character on the left, 5L means end on fifth character from the left
...
**Answer**
Baltimore city was the top recipient for grants and loans with 8,464,101 which makes sense as it's the largest city in the state.

filter(maryland tourism board)
group by
summarize
WHICH ZIP CODE using zip 5, group by zip5
most amt of money - summarize(sum)
