...
Yes but the 2022 data has the Total in mixed cases (caps and lower).
...
NOTES: The column names need to be completely identical including case to bind these
...
The number of registered voters has steadily increased each year, and the county has gained a ton of democratic voters since 2016.
...
NOTE: want dataframe with more ? to go first
Innerjoin includes only everything where one row matches another row - everything in common
join_by uses the column they have in common to match them up (this is what you use to join, key that puts them both together)

**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24, there should be 24!
...
Information on the population 18 and over!! and racial demographics of Maryland residents by county.
...
Prince George's, Baltimore City and Charles counties have the most Black residents age 18 or older per capita.
...
Howard and Montgomery County have the highest proportion of Asian residents. These two also have lower rates of Black residents, although they're about equally represented.
...
Currently it is a double column.
...
Now it's a character column.

NOTE: we can do the same thing in reverse by using as.numeric, don't want to change the name of the column.
