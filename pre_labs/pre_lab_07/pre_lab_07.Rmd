...
Notes: doing the variables = c just names the columns for you
function top_n(20, estimate) shows you the top 20
...
Howard County, MD with an estimated median income of about $129,549 per year.
...
**Answer**
It should be something more like Estimated Median Household Incomes in Maryland Counties - I originally had margin of error in the title but the descriptor at the bottom makes it a bit clearer? I would probably add some labels or a key or something to clarify.
...
It doesn't look like it is because their margin of error bars overlap.

Is the difference between Howard and everyone else significant? **Answer**
Yes, I would say so. I mean it's at least 10k between the second-highest income county and everyone else falls behind that.
...
**Answer**
Baltimore City saw the biggest jump in income from 2010 to 2021. No counties saw a drop in median income, but Caroline County saw a very small increase. 
...
**Answer**
How often do kids from low-income neighborhoods tend to enroll in college right after high school? This might be kind of hard to answer because it will be a bunch of different percentage rates and also varying income levels. I would probably look at the bottom five counties by income and then look at the zips for that maybe?
...
The average median income for high schools where at least 60% of students enrolled in college immediately after graduation is 111,448. High schools where less than 40% of students enrolled directly after high school had an average median income of 69,252. 
```{r}
...
Do counties that saw the highest increase in median income over time send a comparable rate of high school graduates to college right after high school (in line with counties that have similar incomes but didn't see the same change)? Essentially I want to see if that flux in income is represented in the high school college enrollment data.
